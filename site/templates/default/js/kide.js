...
kide.mensaje = function(name, uid, id, url, ti, session, row, img) {
	this.html('KIDE_mensaje_username', name);
	this.attr('KIDE_mensaje_username', 'className', "KIDE_"+kide.rows[row]);
	...
	if (this.row == 1) {
		...
		if (session != '0' && row != 1) {
			...
			this.attr('KIDE_mensaje_banear', 'onclick', function() { kide.banear(session, 'mensaje'); });
		}
		else
...
kide.mostrar_usuario = function(uid, name, row, session, url, img) {
	...
	if (this.row == 1) {
		...
		if (session != '0' && row != 1) {
			...
			this.attr('KIDE_usuario_banear', 'onclick', function() { kide.banear(session, 'usuario'); }); 
		}
		else
...
kide.insertNewContent = function(uid,name,text,url,ti,color,row,id,session,yo,hora,img) {
	if (text.replace(/ /g, "") != "") {
		var c = color.length>0 ? 'style="color:#'+color+'" class="KIDE_msg"' : 'class="KIDE_dc_'+this.rows[row]+' KIDE_msg"';
		...
		nodo.innerHTML = s_hora+tmp+'<span style="cursor: pointer" class="KIDE_'+this.rows[row]+'" onclick="kide.mensaje(\''+name+'\', '+uid+', '+id+', \''+url+'\', \''+ti+'\', \''+session+'\', '+row+', \''+img+'\')">'+name+'</span>: <span '+c+'>'+this.filter_smilies(text)+'</span>';
...
kide.insert_session = function(user) {
	var div = document.createElement('div');
	div.setAttribute('style', 'cursor:pointer');
	div.setAttribute('class', user._class);
	div.onclick = function() { kide.mostrar_usuario(user.id, user.name, user.row, user.session, user.profile, user.img) };
