...
cgchat.mensaje = function(name, uid, id, url, ti, session, row, img) {
	...
	this.attr('KIDE_mensaje_username', 'className', "KIDE_"+cgchat.rows[row]);
	...
	this.attr('KIDE_mensaje_ocultar', 'href', 'javascript:cgchat.show("KIDE_id_'+id+'")');
	if (this.row == 1) {
		this.show('KIDE_mensaje_banear_span', false);
		if (session != '0' && row != 1) {
			this.show('KIDE_mensaje_banear_span1', true);
			this.attr('KIDE_mensaje_banear', 'onclick', function() { cgchat.banear(session, 'mensaje'); });
		}
		else
...
cgchat.mostrar_usuario = function(uid, name, row, session, url, img) {
	...
	if (this.row == 1) {
		...
		if (session != '0' && row != 1) {
			...
			this.attr('KIDE_usuario_banear', 'onclick', function() { cgchat.banear(session, 'usuario'); }); 
		}
		else
...
cgchat.insertNewContent = function(uid,name,text,url,ti,color,row,id,session,yo,hora,img) {
	if (text.replace(/ /g, "") != "") {
		...
		nodo.innerHTML = s_hora+tmp+'<span style="cursor: pointer" class="KIDE_'+this.rows[row]+'" onclick="cgchat.mensaje(\''+name+'\', '+uid+', '+id+', \''+url+'\', \''+ti+'\', \''+session+'\', '+row+', \''+img+'\')">'+name+'</span>: <span '+c+'>'+this.filter_smilies(text)+'</span>';
...
cgchat.show_colors = function() {
	if (!cgchat.html('KIDE_opciones_colores')) {
		...
		for (var i=0; i<colors.length;i++) {
			c = colors[i];
			out += '<a href="javascript:cgchat.set_color(\''+c+'\')"><img class="KIDE_r" src="'+this.img_blank+'" style="background-color:#'+c+'" /></a>';
		}
...
cgchat.ajustar_scroll = function() {
	...
	if (cgchat.order == 'bottom')
		cgchat.attr('KIDE_msgs', 'scrollTop', cgchat.attr('KIDE_msgs', 'scrollHeight'));
	else
		cgchat.attr('KIDE_msgs', 'scrollTop', 0);
};